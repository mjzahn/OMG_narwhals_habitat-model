...
```{r make dataframe for period before fast ice freeze-up}
...
mdl_data_b4freeze2018 <- model_vars_df %>% filter(year==2018) %>% 
  filter(DOY<301) %>% drop_na()
mdl_data_b4freeze2019 <- model_vars_df %>% filter(year==2019) %>% 
  filter(DOY>100,DOY<357) %>% drop_na()

## combine the two years of observations
mdl_data_b4freeze <- rbind(mdl_data_b4freeze2018,mdl_data_b4freeze2019)

## sanity check plot
qplot(as.Date(time),salt_shallow, geom="point", data = mdl_data_b4freeze)

# run correlation test
mdl_data_corr <- mdl_data_b4freeze %>% dplyr::select(!c(narwhal, time, site, year, DOY, device, noise_100Hz, noise_2kHz)) %>% cor(use = "complete.obs")

mdl_data_corr <- mdl_data_b4freeze %>% dplyr::select(!c(narwhal, time, site, year, DOY, device, noise_100Hz, noise_2kHz, salt_deep, temp_deep,velocity,runoff_mar)) %>% cor(use = "complete.obs")
...
```{r select whale occupation period}
...
save(mdl_data_b4freeze, file="data-cleaning_R/narwhal_model_data.rdata")
save(mdl_data_whales, file="data-cleaning_R/narwhal_model_data_whales.rdata")

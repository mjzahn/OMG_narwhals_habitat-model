...
mdl_data <- mdl_data_open_water %>% left_join(ref,by=c("site","year")) %>% mutate(glacier_len_norm = glacier_length - length_ref) %>% dplyr::select(!c(length_ref,noise_100Hz,noise_2kHz,runoff_mar,glacier_length))
...
mdl_global_open_water <- glm(narwhal ~ 
                               ...
                               DOY:temp_surf+
                               DOY:salt_deep+
                               I(DOY^2):I(temp_surf^2)+
                               I(DOY^2):I(salt_deep^2)+
                               DOY:ice_cover_percent+
                               ice_cover_percent:salt_deep+
                               site,
...
mdl_data_open_water_aural <- mdl_data %>% filter(device=='aural')
...
## select only aural data
mdl_data_open_water_soundtrap <- mdl_data %>% filter(device=='soundtrap')
